...
public class PlayerInventoryManager : NetworkBehaviour
{
    ...
    [Command]
    private void CmdWeaponUpdate(bool reload, bool fireDown, bool fireUp, Vector3 direction)
    {
        ...
        else if (fireDown)
        {
            if (weapon.TryFire())
            {
                ...
                ServerSpawnProjectile(projectileId, weapon.ShootTransform.position, direction);
            }
        }
        else if (fireUp)
        {
    ...
    [Server]
    private void ServerSpawnProjectile(int objectId, Vector3 spawnPosition, Vector3 forward)
    {
        GameObject projectileInstance = Instantiate(nm.spawnPrefabs[objectId], spawnPosition, Quaternion.LookRotation(forward, Vector3.up));
    ...
    private void SpawnEquipment()
    {
        ...
        primary.SetCamera(playerCamera);
        secondary.SetCamera(playerCamera);
