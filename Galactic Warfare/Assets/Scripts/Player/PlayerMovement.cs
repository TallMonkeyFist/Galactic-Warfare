...
public class PlayerMovement : NetworkBehaviour
{
    ...
    public class PlayerMoveState
    {
        ...
        public PlayerMoveState() : this(Vector3.positiveInfinity, Vector3.forward, -1, -1, 0, false)
        {
    ...
    private PlayerMoveState ClientLastState = null;
    ...
    private void ClientMovePrediction()
    {
        ...
        for (int i = 0; i < moveCommands.Count; i++)
        {
            ...
            if(moveCommand.moveNumber <= ServerLastMove.moveNumber)
            {
                continue; 
            }
            ...
            moveCommand.position = controller.transform.position;

            if (!moveCommand.simulated)
            {
                moveCommand.simulated = true;
                //Play sound effects
            }

            moveCommands[i] = moveCommand;

            ClientLastState = new PlayerMoveState(
                controller.transform.position, lastDirection, stamina, 
                gravity, moveCommand.moveNumber, sprinting);
        }
    }
